...
class restrict_folder : public IFileDialogEvents {
public:
        std::wstring folder_location;

        virtual HRESULT STDMETHODCALLTYPE OnFileOk( /* [in] */ __RPC__in_opt IFileDialog* pfd) {
                return E_NOTIMPL;
        }
        ...
        virtual HRESULT STDMETHODCALLTYPE OnFolderChanging(
                ...
                bool allow = false;
                wchar_t* display_folder_name = nullptr;
                if(!FAILED(psiFolder->GetDisplayName(SIGDN_FILESYSPATH, &display_folder_name)) && display_folder_name) {
                        std::wstring as_w_string(display_folder_name);
                        allow = as_w_string == folder_location || as_w_string.starts_with(folder_location + L"\\");
                        CoTaskMemFree(display_folder_name);
                }
                if(allow) {
                        return S_OK;
                } else {
...
std::wstring pick_existing_file_from_folder(std::wstring extension, std::wstring const& directory) {
        // CREATE FileOpenDialog OBJECT
        IFileOpenDialog* f_FileSystem;
        auto f_SysHr = CoCreateInstance(CLSID_FileOpenDialog, NULL, CLSCTX_ALL, IID_IFileOpenDialog, reinterpret_cast<void**>(&f_FileSystem));
        if(FAILED(f_SysHr)) {
        ...
        static restrict_folder restrictions;
        restrictions.folder_location = directory;
        if(!restrictions.folder_location.empty() && restrictions.folder_location.back() == L'\\')
                restrictions.folder_location.pop_back();

        IShellItem* pCurFolder = nullptr;
        SHCreateItemFromParsingName(directory.c_str(), nullptr, IID_PPV_ARGS(&pCurFolder));
        if(pCurFolder) {
        ...
        f_FileSystem->Advise(&restrictions, &advise_cookie);
        ...
        //  SUCCESS, CLEAN UP
        f_FileSystem->Unadvise(advise_cookie);
        CoTaskMemFree(f_Path);
        f_Files->Release();
        f_FileSystem->Release();
        return path;
